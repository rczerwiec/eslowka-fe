...
import { RootState, useFetchRandomWordsArrayQuery } from "../../../shared/store";
import { IWord } from "../../../shared/store/slices/FolderSlice";

const WordTranslationTraining = () => {
  const [translation, setTranslation] = useState("");
  const [wordsList, setWordsList] = useState<IWord | any>();
  const folder = useSelector((state: RootState) => state.folderProfile);
  const response = useFetchRandomWordsArrayQuery(folder.id);//getRandomWordFromFolder
      ...
      <>
        <div className="flex flex-col w-full h-full">
          ...
          >
            ...
            <div className="font-bold text-green-600 text-5xl hidden">
              Dobrze!
            </div>
            <div className="font-thin text-5xl">            {randomWord}</div>
