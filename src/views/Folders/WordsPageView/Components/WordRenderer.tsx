...
import { useRemoveWordMutation } from "../../../../shared/store";

export const WordsTable: FC<{renderedWords:JSX.Element | undefined}> = (props): JSX.Element => {
    return(
        <table>
...
const WordRenderer: FC<{data: IWord[], folder: IFolder, setNewID: (newID: number) => void}> = (props): JSX.Element => {
          ...
          <tr className="h-14 font-inter font-medium text-xl" key={word.id}>
            ...
            <th className="flex justify-between items-center h-14 p-4">
              <FaTrashAlt
                ...
                onClick={() => {
                  removeWord({ word: word, folderID: props.folder.id });
                }}
              />
            </th>
          </tr>
