  ...
  if (props.data.length !== 0) {
    tr = props.data.map((word: IWord, index: number) => {
      ...
      if (word.streak >= 5 && word.streak < 15) {
        streakIcon = (
          <FaFire className=" text-2xl text-orange-600 max-lg:text-base" />
        );
      } else if (word.streak >= 15 && word.streak < 35) {
        streakIcon = (
          <FaFire className=" text-2xl text-zinc-400 max-lg:text-base" />
        );
      } else if (word.streak >= 35) {
        streakIcon = (
          <FaFire className=" text-2xl text-gold max-lg:text-base" />
        );
      } else if (word.reverseStreak >= 5) {
        streakIcon = (
          <FaSkull className=" text-2xl text-red-600 max-lg:text-base" />
        );
      } else {
        streakIcon = (
          <FaFire className=" text-2xl text-transparent max-lg:text-base" />
        );
      }

      tr = (
        <tr
          className="h-14 font-inter font-medium text-xl max-lg:text-sm "
          key={word.id}
        >
          ...
          >
            {word.translation.slice(0, 13)}
            {word.translation.length > 20 ? <span>...</span> : <></>}
          </th>
